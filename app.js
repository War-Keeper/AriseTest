/* =========================
   ...
   (regex-free helpers to avoid "missing /" errors)
   ========================= */
...
// strip everything except digits, dot, minus — implemented without regex
...
const parseCurrency = (v) => {
  ...
  const f = parseFloat(stripNonNumeric(v));
  return Number.isFinite(f) ? f : null;
};
const parseNumber = parseCurrency;

// split on comma, semicolon, pipe, CR or LF — implemented without regex
...
// video check without regex
function isVideo(u) {
  const s = (u || "").toLowerCase().split("?")[0];
  return [".mp4", ".webm", ".ogg", ".mov"].some((ext) => s.endsWith(ext));
}
...
/* Quality grade helpers (A..AAAAA) — regex-free string checks */
...
function clarityScore(c) {
  const map = {
    IF: 5, VVS: 4.5, VVVS: 4.5, VS: 4, SI: 3, I1: 2,
    OPAQUE: 1.5, TRANSLUCENT: 2.5, TRANSPARENT: 3,
  };
...
function treatmentPenalty(t) {
  ...
  if (!s || has("none") || has("unheated") || has("no oil") || has("untreated")) return +0.5; // bonus
  if (has("minor oil")) return -0.2;
  if (has("moderate") || has("irradiated") || has("glass") || has("stabilized")) return -0.6;
  return -0.3;
}
...
/* ----- FACETS & TOOLBAR ----- */
const uniq = (list) =>
  [...new Set(list.filter(Boolean).map((v) => String(v).trim()))].sort((a, b) =>
    a.localeCompare(b)
  );
...
function facetSection(id, title, items, renderLabel) {
  ...
  summary.className =
    "cursor-pointer list-none flex items-center justify-between text-sm font-medium text-gray-900 select-none";
...
function renderFacetsInto(containerId, suffix = "") {
  ...
  const setIfEmpty = (id, val) => {
    const el = Q(id + suffix);
    if (el && !el.value) el.value = String(val);
  };
  if (carats.length) {
    setIfEmpty("minCt", Math.min(...carats).toFixed(2));
    setIfEmpty("maxCt", Math.max(...carats).toFixed(2));
  }
  if (totals.length) {
    setIfEmpty("minTotal", Math.floor(Math.min(...totals)));
    setIfEmpty("maxTotal", Math.ceil(Math.max(...totals)));
  }
...
function renderToolbar() {
  const fill = (id, label, items) => {
    ...
    if (!el) return;
    el.innerHTML =
      `<option value="">${label}</option>` + items.map((o) => `<option>${o}</option>`).join("");
  };
...
function renderPills() {
  ...
  if (state.minCt != null)
    addPill("Min ct", state.minCt, () => {
      state.minCt = null;
      ["minCt", "caratMin", "minCtM"].forEach((id) => Q(id) && (Q(id).value = ""));
      applyFilters();
    });
  if (state.maxCt != null)
    addPill("Max ct", state.maxCt, () => {
      state.maxCt = null;
      ["maxCt", "caratMax", "maxCtM"].forEach((id) => Q(id) && (Q(id).value = ""));
      applyFilters();
    });
  if (state.minTotal != null)
    addPill("Min $", state.minTotal, () => {
      state.minTotal = null;
      ["minTotal", "priceMin", "minTotalM"].forEach((id) => Q(id) && (Q(id).value = ""));
      applyFilters();
    });
  if (state.maxTotal != null)
    addPill("Max $", state.maxTotal, () => {
      state.maxTotal = null;
      ["maxTotal", "priceMax", "maxTotalM"].forEach((id) => Q(id) && (Q(id).value = ""));
      applyFilters();
    });
}
...
function renderGrid() {
  ...
  for (const x of slice) {
    ...
    img.src =
      firstMedia ||
      `https://placehold.co/600x600?text=${encodeURIComponent(x.stone || "Gem")}`;
    ...
    card.addEventListener("click", (e) => {
      ...
      toggleDetails(card, x);
    });
    frag.querySelector("[data-toggle]").addEventListener("click", (e) => {
      e.stopPropagation();
      toggleDetails(card, x);
    });
