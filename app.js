...
const COLS = {
  ...
  sizeMm: [
    "Size (MM)", "Size", "Measurements (mm)", "Dimensions", "Dims (mm)",
    "LxWxH (mm)", "Length (mm)", "Width (mm)", "Height (mm)", "Gemstone Dimensions",
  ],
  sizeRange: ["Mix Size Range", "Size Range"],
  caratTotal: [
    "Stone Weight - Carat Total", "Carat", "Weight", "Carat Weight",
    "Weight (ct)", "Total Carat Weight", "TCW", "Gemstone Carat / Weight",
  ],
  ...
  retailPerCt: [
    "Retail Price/Ct", "Retail - Price/Ct", "Retail $/ct",
    "Price per ct", "Price/Ct", "Sale Price",
  ],
...
const parseCurrency = (v) => {
  ...
  const n = String(v).replace(/[^\d.-]/g, "");   // safer
...
/* Quality score (used for A..AAAAA filter) */
...
function treatmentPenalty(t){ const s=(t||"").toLowerCase(); if(!s||/none|unheated|no oil|untreated/.test(s)) return +0.5; if(/minor oil/.test(s)) return -0.2; if(/moderate|irradiated|glass|stabilized/.test(s)) return -0.6; return -0.3; }
...
function showSkeletons(n=12){
  ...
  const tpl=Q("skeletonTpl")?.content; if(!tpl) return;
  for(let i=0;i<n;i++) grid.appendChild(tpl.cloneNode(true));
}
...
function mapRow(row){
  const o = {
    ...
    media: (() => {
      ...
      // safer regex: avoid embedded literal newline in pattern
      const list = (raw ? raw : byAliases(row, COLS.imageUrl))
        .toString()
        .split(/,|;|\||\r?\n/)   // <— replaced /[\n,;|]/ with safe alternation
        .map(s=>s.trim())
        .filter(Boolean);
...
async function loadData(){
  ...
  try{
    ...
    const csvLink=Q("csvLink"); if(csvLink) csvLink.href=firstURL;

    const parts=await Promise.all(SHEET_SOURCES.map(async src=>{
      const url = ALWAYS_FRESH && src.url
    ...
    const grid=Q("grid");
    if(grid) grid.innerHTML="<div class='col-span-full text-center text-sm text-red-600'>Failed to load data. Check your published CSV URL.</div>";
  }
}
...
function renderChips(id,items,onClick,labelRenderer){
  ...
  items.forEach(val=>{ const btn=document.createElement("button"); btn.className="px-2 py-1 rounded-lg border border-gray-300 text-xs bg-white hover:bg-gray-50 flex items-center gap-1"; btn.innerHTML=labelRenderer?labelRenderer(val):val; btn.addEventListener("click",()=>onClick(val)); box.appendChild(btn); });
}
...
function toggleDetails(cardEl,data){
  const box=cardEl.querySelector("[data-details]"); if(!box) return;
  const isOpen=!box.classList.contains("hidden");
  if(isOpen){ box.classList.add("hidden"); } else { buildDetails(cardEl,data); box.classList.remove("hidden"); }
}

/* Filtering & sort */
const toggleSet=(set,val)=>{ set.has(val) ? set.delete(val) : set.add(val); };
const matchesText=(x,q)=>{ if(!q) return true; const hay=`${x.stockId} ${x.originalId} ${x.stone} ${x.color} ${x.shape} ${x.clarity} ${x.origin} ${x.shade}`.toLowerCase(); return hay.includes(q.toLowerCase()); };
...
function readURL(){
  ...
  for(const k of ["stone","color","shape","clarity","origin","shade","status","certificate","treatment","pair"]){
    const v=p.get(k); state[k]=new Set(v ? v.split(",") : []);
  }
  ...
  state.perPage=parseInt(p.get("pp")||"24",10);
...
function renderPills(){
  ...
  for(const k of ["stone","color","shape","clarity","origin","shade","status","certificate","treatment","pair"]){
    for(const val of state[k]) add(k[0].toUpperCase()+k.slice(1),val,()=>{ state[k].delete(val); applyFilters(); });
  }
